    ...
    @if($activeSession)
        ...
        <div class="w-full max-w-lg">
            <div class="text-center mb-8">
                ...
                <p class="text-slate-400 text-sm mt-1">Opened at {{ $activeSession->opened_at->format('H:i') }}</p>
            </div>

            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 mb-6">
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="bg-slate-800 rounded-xl p-4">
                        <p class="text-xs text-slate-500 mb-1">Opening Cash</p>
                        <p class="text-lg font-bold text-white">
                            {{ number_format($activeSession->opening_amount, 0, '.', ' ') }} F</p>
                    </div>
                    <div class="bg-slate-800 rounded-xl p-4">
                        <p class="text-xs text-slate-500 mb-1">Sales Total</p>
                        <p class="text-lg font-bold text-emerald-400">
                            {{ number_format($activeSession->total_sales, 0, '.', ' ') }} F</p>
                    </div>
                </div>
                ...
                <div class="border-t border-slate-700 pt-4">
                    <p class="text-sm font-medium text-slate-300 mb-3">Close Session</p>
                    <div class="flex gap-3">
                        ...
                        <button wire:click="closeSession"
                            ...
                            wire:confirm="Are you sure you want to close this session?">
                            Close Session
                        </button>
                    </div>
                </div>
    ...
    @else
        ...
        <div class="w-full max-w-md">
            <div class="text-center mb-8">
                ...
                <h1 class="text-xl font-bold text-white">Start Your Shift</h1>
                <p class="text-slate-400 text-sm mt-1">Enter your opening cash amount to begin</p>
            </div>

            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                <div class="mb-4">
                    <label class="block text-sm font-medium text-slate-300 mb-2">Opening Cash Amount (F CFA)</label>
                    <input wire:model="openingAmount" type="number" step="0.01" min="0" placeholder="e.g. 50000"
                ...
                <button wire:click="openSession"
                    ...
                    <span wire:loading.remove>Open Cash Session</span>
                    <span wire:loading>Opening...</span>
                </button>

                <p class="text-xs text-slate-600 text-center mt-3">You cannot process sales without an open cash session.
                </p>
            </div>
        </div>
    @endif
