    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
        <div class="bg-gradient-to-br from-indigo-600/20 to-indigo-900/20 border border-indigo-700/30 rounded-2xl p-6">
            <div class="flex items-center justify-between mb-3">
                <p class="text-sm text-slate-400 font-medium">Today's Revenue</p>
            ...
            <p class="text-xs text-slate-500 mt-1">All completed sales</p>
        </div>

        <div
            class="bg-gradient-to-br from-emerald-600/20 to-emerald-900/20 border border-emerald-700/30 rounded-2xl p-6">
            <div class="flex items-center justify-between mb-3">
                <p class="text-sm text-slate-400 font-medium">Sales Today</p>
            ...
            <p class="text-xs text-slate-500 mt-1">Transactions processed</p>
        </div>

        <div class="bg-gradient-to-br from-amber-600/20 to-amber-900/20 border border-amber-700/30 rounded-2xl p-6">
            <div class="flex items-center justify-between mb-3">
                <p class="text-sm text-slate-400 font-medium">Products</p>
            ...
            <p class="text-xs text-slate-500 mt-1">Items in catalog</p>
        </div>

        <div class="bg-gradient-to-br from-violet-600/20 to-violet-900/20 border border-violet-700/30 rounded-2xl p-6">
            <div class="flex items-center justify-between mb-3">
                <p class="text-sm text-slate-400 font-medium">Active Users</p>
            ...
            <p class="text-xs text-slate-500 mt-1">Staff members</p>
        </div>
    </div>
        ...
        <div class="xl:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
                ...
                <a href="{{ route('reports') }}"
                    class="text-xs text-indigo-400 hover:text-indigo-300 transition-colors">View all →</a>
            </div>
            <div class="divide-y divide-slate-800">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500">{{ $sale->created_at->format('H:i') }} ·
                                    {{ $sale->items->count() }} item(s)</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-semibold text-white">
                                {{ number_format($sale->total_amount, 0, '.', ' ') }} F</p>
                ...
                @empty
                    <div class="px-6 py-8 text-center text-slate-500 text-sm">No sales today yet.</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-800">
                    <h2 class="text-sm font-semibold text-white">Open Sessions</h2>
                </div>
                <div class="divide-y divide-slate-800">
                    @forelse($this->openSessions as $session)
                        <div class="px-6 py-3">
                            ...
                            <p class="text-xs text-slate-500">Since {{ $session->opened_at->format('H:i') }}</p>
                            <p class="text-xs text-emerald-400 font-medium mt-0.5">
                                {{ number_format($session->total_sales, 0, '.', ' ') }} F sold</p>
                        </div>
                    @empty
                        <div class="px-6 py-5 text-center text-slate-500 text-sm">No active sessions.</div>
                    @endforelse
                </div>
            </div>
            ...
            @if($this->lowStockProducts->count() > 0)
                <div class="bg-slate-900 border border-amber-800/30 rounded-2xl overflow-hidden">
                    <div class="px-6 py-4 border-b border-amber-800/30 flex items-center gap-2">
                        ...
                        <h2 class="text-sm font-semibold text-amber-400">Low Stock Alert</h2>
                    </div>
                    <div class="divide-y divide-slate-800">
                        @foreach($this->lowStockProducts as $product)
                            <div class="px-6 py-3 flex items-center justify-between">
                                ...
                                <span
                                    class="text-xs font-bold text-red-400 bg-red-900/30 px-2 py-0.5 rounded-full">{{ $product->stock }}
                                    left</span>
                            </div>
                        @endforeach
                    </div>
